...
Element = quick_draw_data_set.length[random_no]
console.log(Element)
time_counter = 0
time_check = ""
drawn_sketch = ""
...
function gotResult(error,results){
    ...
    console.log(results);
    drewn = results[0].label
    document.getElementById('hi').innerHTML = "Your sketch is a "+ drewn;
    document.getElementById('acuracy').innerHTML = 'Percentage:' + Math.round(results[0].confidence *100)+'%'
...
function checkskect(){
    ...
    if(time_counter >400||time_counter == 400){
        time_counter = 0;
...
function updateCanvas(){
    background('white')
    random_no = Math.floor((Math.random()*quick_draw_data_set.length)+1);
    x=quick_draw_data_set[random_no];
    console.log(x);
    console.log(random_no);
    document.getElementById("random_draw").innerHTML = x;
}
